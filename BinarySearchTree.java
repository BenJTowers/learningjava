...
class BinarySearchTree <K extends Comparable<K>, V>  {
	...
	private void insertRec(K key, V value, BSTNode<K,V> cur)
	{
	...
	private V findRec(BSTNode<K,V> cur, Comparable<K> key) throws KeyNotFoundException
	{
		if(cur == null)
		{
	...
	private int heightRec(BSTNode<K,V> cur)
	{
		...
		return ((Math.max(heightRec(cur.left), heightRec(cur.right))) + 1);
	}
	...
	public List<Entry<K,V>>	entryList() {
		...
		{
			BSTNode<K,V> tempNode = q.removeFirst();
			l.add(new Entry<K,V>(tempNode.key, tempNode.value));
