...
class BinarySearchTree <K extends Comparable<K>, V>  {
	...
	private void insert(K key, V value, BSTNode<K,V> cur){
	...
	private V find(BSTNode<K,V> cur, Comparable<K> key) throws KeyNotFoundException{
	...
	private int height(BSTNode<K,V> cur){
		if(cur == null){
			return -1;
		}
	...
	public List<Entry<K,V>>	entryList() {
		...
		while(q.isEmpty() != true){
			BSTNode<K,V> tnode = q.removeFirst();
			l.add(new Entry<K,V>(tnode.key, tnode.value));
			if(tnode.left != null){
